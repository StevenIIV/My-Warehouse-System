...
public class ReturnWarrant extends JPanel implements ActionListener{
    ...
    public void setInitTable(){
        try {
            goods= sw.selectReturnWarrant();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        if(goods.isEmpty()){
            JOptionPane.showMessageDialog(null, "退料单数据缺失", "ERROR!",JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
        maxpage=goods.size();
        for(int i=0;i<20;i++){
