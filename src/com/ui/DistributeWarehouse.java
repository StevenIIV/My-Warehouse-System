...
public class DistributeWarehouse extends JPanel implements ActionListener{
    ...
    Object[][] titleName={{"仓库名",null},{"货物编号",null},{"数量",null},{"单价",null},{"总价",null}};
    public DistributeWarehouse(){
        initComponents();
    }
    ...
    public void setInitTable(){
        try {
            goods= sw.selectDistributeWarehouse();
        } catch (SQLException e) {
            e.printStackTrace();
        }
        if(goods.isEmpty()){
            JOptionPane.showMessageDialog(null, "本仓数据缺失", "ERROR!",JOptionPane.ERROR_MESSAGE);
            System.exit(1);
        }
        maxpage=goods.size();
        for(int i=0;i<5;i++){
